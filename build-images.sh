...
# Reuse existing nodebuilder-espocrm container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-espocrm; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-espocrm -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
